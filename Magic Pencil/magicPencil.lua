...
function MagicPencil:Execute()
    ...
    Mode(Modes.Regular, "常规模式", true, true)

    self.dialog:separator{text = "轮廓生成"} --
    Mode(Modes.Outline, "封闭图形")
    Mode(Modes.OutlineLive, "跟随笔刷")
    self.dialog:color{
        id = "outlineColor",
        visible = false,
    ...
    self.dialog:separator{text = "变换"} --
    Mode(Modes.Cut, "上移")
    Mode(Modes.Selection, "选择区")

    -- self.dialog:separator{text = "Forbidden"} --
    Mode(Modes.Yeet, "Yeet", false)

    self.dialog:separator{text = "混合方式"}
    Mode(Modes.Mix, "唯一混合")
    Mode(Modes.MixProportional, "成比混合")

    self.dialog:separator{text = "改变"} --
    Mode(Modes.Colorize, "鲜艳化")
    Mode(Modes.Desaturate, "去饱和")

    self.dialog:separator{text = "转变"} --
    :combobox{
        id = "colorModel",
        options = ColorModels,
    ...
    self.dialog --
    :separator() --
    :check{id = "indexedMode", text = "索引模式"}
