...
function SpriteAnalyzerDialog:Create(options)
        ...
        onclick = function()
            self:LoadConfiguration({}, function()
                ...
                local function ShowOutlineExtractionDialog()
                    ...
                    local outlineExtractionDialog = Dialog("轮廓颜色")
                    outlineExtractionDialog --
                    :label{text = "这些是正确的轮廓颜色吗？"} --
                    :shades{
                        id = "outline-colors",
                        mode = "sort",
                ...
                local outlineConfirmationDialog = Dialog("轮廓？") --
    ...
    :button{
        text = "保存",
        onclick = function()
            local savePresetDialog = Dialog("保存预设")
            savePresetDialog --
            :entry{
                id = "preset-name",
                text = self.name,
    ...
    :button{
        ...
        onclick = function()
            ...
            local loadPresetDialog = Dialog("加载预设")
            loadPresetDialog --
            :combobox{id = "preset", label = "预设：", options = presetNames} --
            :button{
                id = "load-button",
                text = "加载",
...
function SpriteAnalyzerDialog:LoadConfiguration(configuration, onload)
    if self.modified then
        local confirmationDialog = Dialog("未保存的更改")

        confirmationDialog --
        :label{text = "未保存的更改将丢失，您要继续吗？"} --
        :button{
            text = "是",
            onclick = function()
